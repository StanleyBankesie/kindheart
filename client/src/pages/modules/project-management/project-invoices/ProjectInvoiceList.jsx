/**
 * @fileoverview ProjectInvoiceList component.
 * Provides functionality for ProjectInvoiceList.
 */
...
export default function ProjectInvoiceList() {
  ...
  const fetchAll = async (page = 1) => {
    try {
      ...
      const requests = [api.get(`/projects/project-invoices?page=${page}&limit=50`)];
...
<html>
    ...
    return `
      ...
      <div class="doc">
        <div class="doc-header">
          ...
          <div class="meta">
            <div class="doc-title">Project Invoice</div>
            <div>Invoice No: ${escapeHtml(inv.invoice_no || "")}</div>
            <div>Project Invoice Date: ${escapeHtml(inv.invoice_date || "")}</div>
  ...
  const openForwardModal = async (doc) => {
    ...
    try {
      ...
      const active = wfs.find((w) => Number(w.is_active) === 1 && (String(w.document_type).toUpperCase() === "SALES_INVOICE" || String(w.document_type).toUpperCase() === "SALES INVOICE" || String(w.document_route) === "/projects/project-invoices"));
  ...
  async function handleForwardSubmit() {
    ...
    try {
      ...
      const res = await api.post(`/projects/project-invoices/${selectedDoc.id}/submit`, {
      ...
      <div className="card">
        <div className="card-header bg-brand text-white rounded-t-lg">
          <div className="flex justify-between items-center text-white">
            <div>
              <h1 className="text-2xl font-bold dark:text-brand">
                Project Invoices
              </h1>
            ...
            <div className="flex gap-2">
              ...
              <Link to="/project-management/project-invoices/new" className="btn-success">
                + New Invoice
              </Link>
            </div>
          </div>
                      ...
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <div className="min-w-[80px]">
                          <button
                            ...
                            onClick={() => navigate(`/project-management/project-invoices/${inv.id}?mode=view`)}
                          >
                            View
                          </button>
                        </div>
                        ...
                        <div className="min-w-[80px]">
                          ...
                          canPerformAction("sales:invoices", "edit") ? (
                            <button
                              type="button"
                              className="w-full inline-flex items-center justify-center px-4 py-1.5 text-sm font-medium text-slate-700 bg-slate-100 border border-slate-200 rounded-lg hover:bg-slate-200 transition-colors h-9"
                              onClick={() => navigate(`/project-management/project-invoices/${inv.id}?mode=edit`)}
                            >
                              Edit
                            </button>
                          ) : (
                        ...
                        {/* Slot 7: Cancel */}
                        <div className="min-w-[80px]">
                          {exceptionalPerms?.has?.("PM.INVOICE.CANCEL") ? (
                            <button
                              type="button"
                              className="list-approval-reverse-btn"
                              onClick={async () => {
                                ...
                                try {
                                  await api.post(`/projects/project-invoices/${inv.id}/reverse-accounting`);
