/**
 * @fileoverview CreditorsLedgerReportPage component.
 * Standard Modern UI Creditors Ledger report for tracking supplier balances & movements.
 */
...
export default function CreditorsLedgerReportPage() {
  ...
  const filteredAccounts = useMemo(() => {
    const creditorsAccounts = (accounts || []).filter((a) => {
  ...
  async function run() {
    try {
      ...
      toast.error(e?.response?.data?.message || "Failed to load creditors report");
    } finally {
      setLoading(false);
    }
  }
  ...
  const handleExportPDF = () => {
    ...
    let y = 15;
    doc.setFontSize(14);
    doc.text("Creditors Ledger Report", 10, y);
    y += 8;
    doc.setFontSize(9);
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <h1 className="text-2xl font-bold flex items-center gap-2">
                <Building2 className="w-7 h-7" /> Creditors Ledger
              </h1>
              <p className="text-sm mt-0.5 opacity-90">
                Supplier ledger movements, payables tracking and running balance statement
              </p>
            </div>
            ...
            <div className="relative">
              <div className="relative">
                <input
                  ...
                  placeholder={accountId ? selectedAccountLabel || "Search creditor account..." : "All Creditor Accounts (Type to filter...)"}
                  value={accountQuery}
