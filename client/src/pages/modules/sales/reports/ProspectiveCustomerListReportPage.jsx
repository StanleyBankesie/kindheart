/**
 * @fileoverview ProspectiveCustomerListReportPage component.
 * Provides functionality for ProspectiveCustomerListReportPage.
 */
...
export default function ProspectiveCustomerListReportPage() {
  ...
  function exportExcel() {
    ...
    const ws = XLSX.utils.json_to_sheet(
      items.map((r) => ({
        "Prospect Code": r.customer_code || "-",
        "Prospect Name": r.customer_name || r.prospect_customer || "-",
        "Prospect Type": r.customer_type || "-",
        "Email": r.email || "-",
    ...
    XLSX.utils.book_append_sheet(wb, ws, "ProspectiveCustomers");
    ...
    <div className="space-y-4">
      <div className="card shadow-sm border-0">
        <div className="card-header bg-brand text-white rounded-t-lg flex justify-between items-center py-3">
          <div>
            <h1 className="text-xl font-bold">Prospective Customer List Report</h1>
                ...
                {loading ? (
                  ...
                  sorted_items.map((r) => (
                    <tr key={r.id || Math.random()} className="hover:bg-slate-50 dark:hover:bg-slate-900/30">
                      ...
                      <td className="p-3 border-b">{r.phone || r.telephone || "-"}</td>
