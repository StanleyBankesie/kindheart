...
export default function StockAdjustmentList() {
  ...
  const fetchAdjustments = async (currentPage) => {
    setLoading(true);
    setError("");
    try {
      ...
      setError(e?.response?.data?.message || "Failed to load stock adjustments");
    } finally {
      setLoading(false);
    }
  };
  ...
  const forwardDocument = async () => {
    ...
    try {
      ...
      try {
        toast.success("Stock adjustment forwarded for approval");
      } catch {}
    } catch (e) {
      setWfError(
          ...
          {totalPages > 1 && (
            <div className="flex justify-between items-center mt-4 p-4 bg-base-100 rounded-lg shadow-sm border border-base-200">
              <span className="text-sm text-base-content/70">
                ...
                {totalCount > 0 && ` (${totalCount} total adjustments)`}
              </span>
              <div className="join">
                <button
