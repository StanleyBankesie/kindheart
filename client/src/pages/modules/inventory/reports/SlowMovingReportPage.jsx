/**
 * @fileoverview SlowMovingReportPage component.
 * Provides functionality for SlowMovingReportPage.
 */
...
export default function SlowMovingReportPage() {
  ...
  async function run() {
    try {
      setLoading(true);
      const res = await api.get("/inventory/reports/slow-moving", {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm mt-1">Items with low turnover in the period</p>
        </div>
            ...
            <table className="table table-fixed">
              <thead className="sticky top-0 z-10">
                <tr>
                  ...
                  <SortableHeader label="Issued Qty" sortKey="issued_qty" currentKey={sortKey} direction={sortDir} onToggle={toggle} className="text-right" />
                  <SortableHeader label="Turnover" sortKey="turnover" currentKey={sortKey} direction={sortDir} onToggle={toggle} className="text-right" />
                </tr>
              </thead>
              <tbody>
                {sorted_items.map((r) => (
                  <tr key={r.item_id}>
                    ...
                    <td className="text-right">
                      {Number(r.issued_qty || 0).toLocaleString()}
                    </td>
                    <td className="text-right">
                      {Number(r.turnover || 0).toLocaleString()}
                    </td>
                  </tr>
                ))}
              </tbody>
