/**
 * @fileoverview PendingGrnToBillLocalReportPage component.
 * Provides functionality for PendingGrnToBillLocalReportPage.
 */
 ...
 */
export default function PendingGrnToBillLocalReportPage() {
  ...
  async function run() {
    try {
      setLoading(true);
      setError("");
      const res = await api.get("/purchase/reports/pending-grn-to-bill-local");
  ...
  function exportExcel() {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "PendingGRNLocal");
    XLSX.writeFile(wb, "pending-grn-to-bill-local.xlsx");
  }
  ...
  function exportCSV() {
    ...
    a.href = url;
    a.download = "pending-grn-to-bill-local.csv";
    a.click();
  ...
  function exportPDF() {
    ...
    let y = 15;
    doc.setFontSize(14);
    doc.text("Pending GRN -> Purchase Bill (Local)", 10, y);
    y += 8;
    doc.setFontSize(10);
    ...
    doc.save("pending-grn-to-bill-local.pdf");
  }
